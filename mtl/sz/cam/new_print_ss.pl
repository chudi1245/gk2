...
####if (not -d "$path/ps") {	mkpath("$path/ps") or die $!; }
$f->COM("copy_entity,type=wheel,source_job=genesislib,source_name=233.apr,dest_job=$JOB,dest_name=233.apr,dest_database=");
...
if ( exists_layer('gto')  eq  'yes' ) {
	...
	if ( exists_layer('gtl') ) { 
		...
		clear('gtl');
		filter( {feat_types=>'pad', polarity=>'positive', include_syms=>'r80.315'} );
	    ...
	    clear('ref-t');
	    sel_move_other('c');
        output_layer('c',"$path/ps",'no',1,1,0,0,270,'233.apr');#### out c
	};
}
...
if ( exists_layer('gbo') eq 'yes' ) {
	...
	if ( exists_layer('gbl') ) { 
	    ...
	    clear('ref-b');
	    sel_move_other('s');
        output_layer('s',"$path/ps",'yes',1,1,0,0,270,'233.apr');#### out s mirror ײӡַ
