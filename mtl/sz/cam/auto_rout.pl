...
###p__("$nowdate");
#if ( $nowdate > 130916 ) {
#	p__("Free time has passed, please contact the author.");  exit;
#}

$f->COM('units', type => 'mm');
if ($STEP ne 'pnl') { p__("the step not pnl");  exit;  }

##my $path = 'D:/work/pcb/cy';
...
open (FH,"> c:/genesis/fw/jobs/$JOB/output/cy.log") or die $!;
print FH "yes";
close FH;
...
=head
	$f->COM('chain_list_reset');
	$f->COM('chain_set_plunge', layer    => $cy_layer,
                          type           => 'open',
                          mode           => 'chamfer',
                          inl_mode       => 'straight',
                          start_of_chain => 'yes',
