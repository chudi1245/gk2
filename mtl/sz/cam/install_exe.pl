...
open (FH,"> C:/genesis/e97/all/scr_start.csh") or die "$!";
print FH $scr_start;
close FH;
print "rewrite 1 ok\n";
...
my $bindings= << 'EOF_2';
   ...
   BINDING {
       KEY=F2
       SCRIPT=h:/xxx/camp/exe/creat_job.exe
   }
   BINDING {
       KEY=F3
       SCRIPT=h:/xxx/camp/exe/auto_inport_job.exe
   }
   BINDING {
       KEY=F4
       SCRIPT=h:/xxx/camp/exe/clear.exe
   }
   BINDING {
       KEY=F5
       SCRIPT=h:/xxx/camp/exe/dis_next_lay.exe
   } 
   BINDING {
       KEY=F6
       SCRIPT=h:/xxx/camp/exe/select_npth.exe
   } 	   
   BINDING {
       KEY=F7
       SCRIPT=h:/xxx/camp/exe/select_pad.exe
   } 
   BINDING {
       KEY=F8
       SCRIPT=h:/xxx/camp/exe/checklists.exe
   }    
   BINDING {
       KEY=F9
       SCRIPT=h:/xxx/camp/exe/tk_cam.exe
   }
   BINDING {
       KEY=F10
       SCRIPT=h:/xxx/camp/exe/change_unit.exe
   }
   BINDING {
       KEY=F11
       SCRIPT=h:/xxx/camp/exe/cop_orig.exe
   }
   BINDING {
       KEY=F12
       SCRIPT=h:/xxx/camp/exe/open_features.exe
   }
   BINDING {
       KEY=<Shift>F2
       SCRIPT=h:/xxx/camp/exe/break.exe
   }
   BINDING {
       KEY=<Shift>F3
       SCRIPT=h:/xxx/camp/exe/contourize.exe
   }
...
EOF_4
open ( FH_4,  "> C:/genesis/e97/all/perl/Genesis.pl" ) or die "$!";
...
cdr14_default_disp_sr=yes
edt_affect_confirm=yes
edt_cont_accuracy=0.25
edt_cont_brk2islands=yes
edt_cont_stop_on_polarity_mismatch=no
edt_max_undo=30
edt_sr_panel_util_by_active=no
et_extended_udl_file=yes
et_pin_shaft_size=3
et_snap_max_dist=15
get_confirm_undo=yes
get_inp_exclude=*tar;*zip;*tgz;*exe;*gz;*rar;*doc;*xls;*jpg;*pdf;*pcb
gns_form_text_chop=yes
gui_auto_accept=yes
gui_auto_focus=yes
gui_focus_on_option=no
gui_color_no_mix=hsr
iol_274x_set_octagon_rotation=yes
iol_gbr_brk_diag_sqrs=yes
iol_gbr_polygon_break=2
iol_img_rnd_prod_params=yes
out_adjacency_val=18
out_atp_apr_max_size=499
out_new_drill_process=yes

EOF_5

open (FH_2,"> $ENV{USERPROFILE}/.genesis/config") or warn "$!";
print FH_2  $config;
close FH_2;

open (FH_2,"> C:/genesis/sys/config") or warn "$!";
print FH_2  $config;
close FH_2;
print "rewrite 5 ok\n";
