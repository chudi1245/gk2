...
my @title=qw(阻抗层 类型 上参考层 下参考层 线宽 差分线距 共面间距 阻抗值 补偿值 排 列 绕线 Dif2sig);
foreach(0..$#title) {
	my $width;($_ > 3) ? ($width=7) : ($width=10);
    $mw->Label(-text=>$title[$_],-relief=>'sunken',-width=>$width)->grid(-column=>$column++,-row=>$row);  
} ;  $row++;
foreach  (@number_list) {
	$mw->Optionmenu(-options =>\@layer,-variable=>\$imp_layer[$_],-width=>4,)->grid(-column=>0,-row=>$row++); 
	$button_type[$_]=$mw->Button(-text=>'dif',-width=>10,-command=>[\&chang_type,$_])->grid(-column=>1,-row=>$row-1);
    $mw->Optionmenu(-options =>\@layer,-textvariable=>\$ref_top[$_],-width=>4)->grid(-column=>2,-row=>$row-1);
	$mw->Optionmenu(-options =>\@layer,-textvariable=>\$ref_bot[$_],-width=>4)->grid(-column=>3,-row=>$row-1);
	$mw->Entry(-textvariable=>\$line_width[$_],-width=>6)->grid(-column=>4,-row=>$row-1);
	$entry_space[$_]=$mw->Entry(-textvariable=>\$line_space[$_],-width=>6,)->grid(-column=>5,-row=>$row-1);
	$mw->Entry(-textvariable=>\$cu_space[$_],-width=>6,)->grid(-column=>6,-row=>$row-1);
	$mw->Entry(-textvariable=>\$imp_value[$_],-width=>6)->grid(-column=>7,-row=>$row-1);
	$mw->Entry(-textvariable=>\$expiate[$_],-width=>6)->grid(-column=>8,-row=>$row-1);
	$mw->Entry(-textvariable=>\$pos_col[$_],-width=>6)->grid(-column=>9,-row=>$row-1);
	$mw->Entry(-textvariable=>\$pos_row[$_],-width=>6)->grid(-column=>10,-row=>$row-1);
	$checkbutton_u[$_]=$mw->Checkbutton(-width=>4,-state=>'disabled',-variable =>\$u_sig[$_])->grid(-column=>11,-row=>$row-1);
}
$franm->Label(-textvariable=>\$error,-width=>120,-height=>1,-relief=>'ridge')->pack(-side => 'top' ,-fill=>'x');
$franm->Label(-text=>'阻抗条长',-width=>13,)->pack(-side => 'top',-anchor => 'w' );
$franm->Entry(-textvariable=>\$long,-width=>10,)->pack(-side => 'top' ,-anchor => 'w'); 
$franm->Button(-text=>'刷新',-width=>10,-command=>\&brush,)->pack(-side => 'top' ,-anchor => 'w');
$franm->Button(-text=>'Apply',-width=>10,-command=>\&apply,)->pack(-side => 'top' ); 
MainLoop;
...
sub chang_type{
	...
	if ($text eq 'sig') {
		$button_type[$id]->configure(-text=>'dif');
		$entry_space[$id]->configure(-state=>'normal');
 		$u_sig[$id]=undef;
		$checkbutton_u[$id]->configure(-state=>'disabled');
	}else{
		$button_type[$id]->configure(-text=>'sig');
		$line_space[$id]=undef;
		$entry_space[$id]->configure(-state=>'disabled');
		$checkbutton_u[$id]->configure(-state=>'normal');
	}
}
sub brush {
	foreach  (@number_list) {
		...
		$ref_top[$_]=$layer[0];
		$ref_bot[$_]=$layer[0];
		$button_type[$_]->configure(-text=>'dif');
		$line_width[$_]=undef;
		$entry_space[$_]->configure(-state=>'normal');
		$line_space[$_]=undef;
		$cu_space[$_]=undef;
		$imp_value[$_]=undef;
		$expiate[$_]=undef;
		$pos_col[$_]=undef;
		$pos_row[$_]=undef;
		$checkbutton_u[$_]->configure(-state=>'disabled');
		$u_sig[$_]=undef;
		$error=undef;
	}
}
...
sub break_resize {
    clear();
	##$f->COM ('affected_layer',mode=>'all',affected=>'yes');
	affected_layer('yes','single','gts','gbs','drl');
    $f->COM ('sel_break');
    clear();
	affected_layer('yes','single','gts','gbs');
    $f->COM ('sel_resize',size=>4,corner_ctl=>'no');
	clear('drl');
    $f->COM ('sel_change_sym',symbol=>'r47.244',reset_angle=>'no');
}
sub add_sig_imp{  
	my $i=shift;
	...
	my $excursion=0.07;
	my $imp_line_x=$x+$excursion/2;
	my @imp_line_y=($y+0.135, $imp_long-$y-0.135);

	clear($layer);
	if ($cu_space) {
		add_line($imp_line_x, $y+0.035,$imp_line_x, $imp_long-$y-0.035, 's101') ;
		my $neg_symbol='r'.($line_width+$cu_space*2-2*$expiate[$i]);
		add_line($imp_line_x, $imp_line_y[0], $imp_line_x, $imp_line_y[1], "$neg_symbol",'no','negative');
		add_pad( $x,$y,'imp_sig_neg');
        add_pad( $x+$excursion,$imp_long-$y,'imp_sig_neg','no',180);
	}
   
	affected_layer('yes','single','gts','gbs','drl','gtl','gbl');
	add_pad( $x,$y,"imp_sig");
	add_pad( $x+$excursion,$imp_long-$y,"imp_sig",'no',180);

	clear();
	affected_layer('yes','single', @layer[2..$#layer-1]);
	affected_layer('no','single', $layer);
	add_pad( $x,$y,'imp_sig_neg_oth');
	add_pad( $x+$excursion,$imp_long-$y,'imp_sig_neg_oth','no',180);
	clear($layer);
	...
	if ( $u_sig ){
		...
		add_line($imp_line_x, $imp_line_y[0], $imp_line_x, $imp_line_y[1], "r$line_width");
	};
...
sub check_parameter{
	...
	foreach  (@number_list) {
		if ($imp_layer[$_]) {
			...
			$row='z' unless $row;
			my $col_row=$col.$row;
			if (  grep m{$col_row},@pos  ) {  $error="第 $j 组位置重复  ";  return 'bad'}
			push @pos,$col_row;
		}
	}
...
sub apply { 
if (check_parameter() eq 'bad') { return; }
creat_clear_layer('tmp');
clear_creat_step('impedance');	
unit_set('inch');
my (%col_type,$pre_symbol_half,$symbol,$symbol_half,$x,$y);
$imp_long=$long-0.05;
foreach (reverse @number_list){   
	$col_type{$pos_col[$_]}=$button_type[$_]->cget(-text) if $imp_layer[$_] ;
	$max_row=$pos_row[$_] if $pos_row[$_] > $max_row ;
} 
foreach  (sort keys %col_type) {
	( $col_type{$_} eq 'sig' ) ? ( $symbol_half=0.075 ) : ( $symbol_half=0.175 );
...
foreach  (@number_list) {
	...
	if ( $imp_layer[$_]  ) {
	    if  ( $symbol eq 'sig' ) {  add_sig_imp($_)  } else {  add_dif_imp($_)  };
	}
}
break_resize();
...
my $ref=info('step',"$JOB/impedance",'limits');
