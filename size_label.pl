...
###line2lien  pad2pad  line2pad  pad2line
$end_x=( -1*$j_b + sqrt($j_b**2 - 4*$j_a*$j_c) ) / (2*$j_a)  ;  ##quadratic equation of one variable


=head
	($sure_add_dec > 0) ? (
		($kk > 0) ? (   $end_x=( -1*$j_b - sqrt($j_b**2 - 4*$j_a*$j_c) ) / (2*$j_a)
		): ($end_x=( -1*$j_b + sqrt($j_b**2 - 4*$j_a*$j_c) ) / (2*$j_a)
		)
	):(
		($kk > 0) ? (   $end_x=( -1*$j_b + sqrt($j_b**2 - 4*$j_a*$j_c) ) / (2*$j_a)
		) : ($end_x=( -1*$j_b - sqrt($j_b**2 - 4*$j_a*$j_c) ) / (2*$j_a)
...
sub sel_single_feat ($$) {
    ...
    $f->COM ('sel_single_feat',
		...
		$distance=dis_pp($start_x,$start_y,$x1,$y1);
		my $text=round( ($distance-$r)*$I2M,2).'MM';
		my ($a,$b,$c)=($k_t**2+1, -2*($k_t**2+1)*$x1, ($k_t**2+1)*($x1**2)-$r**2  );
		my $tmp =sqrt($b**2 - 4*$a*$c);  ##b^2 -or+ 4ac
		if (  abs( (-$b + $tmp)/2/$a-$start_x ) > abs( (-$b - $tmp)/2/$a-$start_x ) ) {
			$end_x=(-$b - $tmp)/(2*$a);
		}else{
			$end_x=(-$b + $tmp)/(2*$a);
		}
		$end_y=$k_t*$end_x-$k_t*$x1+$y1
